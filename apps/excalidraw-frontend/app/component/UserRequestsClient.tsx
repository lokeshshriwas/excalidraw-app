...
const UserRequestsClient: React.FC = () => {
  ...
  const cancelJoinRequest = async (requestId: string) => {
    if (
      !token ||
      !confirm("Are you sure you want to cancel this join request?")
    )
      return;
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-3">
                          <span
                            className={`px-2 py-1 rounded-md text-xs font-medium flex items-center ${getStatusColor(request.status)}`}
                          >
                      ...
                      <div className="flex items-center space-x-2 ml-4">
                        ...
                        {request.status === "APPROVED" && (
                          <button
                            onClick={() =>
                              router.push(`/canvas/${request.room.slug}`)
                            }
            ...
            {myRooms.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {myRooms.map((room) => (
                  <div
                    key={room.id}
                    className="bg-[#1a1a1a] border border-gray-800 rounded-xl p-6 hover:border-gray-700 transition-colors duration-200"
                  >
